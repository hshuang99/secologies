...
const postsCollection = defineCollection({
  ...
  schema: z.object({
    ...
    // i18n: which language this post is written in (defaults to siteConfig.language if omitted)
    lang: z.string().nullable().optional(),
    // i18n: slugs of this post's translations in other languages, e.g. ["perfect-secrecy-en"]
    translations: z.array(z.string()).nullable().optional().default([]),
  }),
});
...
// Define schema for podcasts
const docsCollection = defineCollection({
  loader: glob({ pattern: '**/*.{md,mdx}', base: './src/content/podcasts' }),
  schema: z.object({
    title: z.string().default('Untitled Podcasts'),
    description: z.string().nullable().optional().default('No description provided'),
...
// Define schema for special home pages (homepage blurb, 404, projects index, podcasts index)
