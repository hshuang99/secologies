...
export const GET: APIRoute = async () => {
  ...
  // Filter projects and podcasts based on environment and optional content type settings
  ...
  const visibleDocs = siteConfig.optionalContentTypes.podcasts
    ? podcasts.filter((doc) => shouldShowContent(doc, isDev) && !doc.data.noIndex)
    : [];
  ...
  // Podcasts index page (only if podcasts are enabled)
  if (siteConfig.optionalContentTypes.podcasts) {
    urls.push(`
      <url>
        <loc>${siteUrl}podcasts/</loc>
        <lastmod>${new Date().toISOString()}</lastmod>
        <changefreq>weekly</changefreq>
        <priority>0.7</priority>
      </url>
