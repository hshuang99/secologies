...
// Generate SEO data for podcast
export function generatePodcastSEO(
  podcast: Podcast,
  url: string
): SEOData {
  const { title, description, image, category, version } = podcast.data;
  ...
  if (image) {
    ...
    ogImage = {
      url: imageUrl,
      alt:
        podcast.data.imageAlt ||
        `Featured image for podcast: ${title}`,
      width: 1200,
      height: 630,
    };
  } else {
  ...
  return {
    title: `${title} | ${siteConfig.title}`,
    description: description || `Podcasts: ${title}`,
    canonical: url,
    ogImage,
    ogType: "article",
    articleSection: category || undefined,
    keywords: version
      ? [`${category || "Podcasts"}`, `version ${version}`]
      : [category || "Podcasts"],
    noIndex: podcast.data.noIndex || false,
  };
}
