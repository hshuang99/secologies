...
function isInternalLink(url: string): boolean {
  ...
  // - Starts with /posts/, /pages/, /projects/, /podcasts/, /special/ (relative URLs)
  // - Starts with posts/, pages/, projects/, podcasts/, special/ (relative paths)
...
function mapRelativeUrlToSiteUrl(url: string): string {
  ...
  if (url.startsWith("/special/")) {
    ...
    } else if (specialPath === "podcasts") {
      return "/podcasts"; // Podcasts index
    } else {
 ...
 * - Podcasts: [Doc Title](podcasts/doc-slug)
          ...
          // Convert .md file references to proper URLs based on collection
            ...
            if (linkText === "homepage") {
              ...
              if (specialPath === "home") {
                ...
                baseUrl = "/podcasts"; // Podcasts index
              } else {
                // Other special pages - use normal page routing
                baseUrl = `/${specialPath}`;
              }
            ...
            } else if (strippedUrl.startsWith("podcasts/") || strippedUrl.startsWith("/podcasts/")) {
              // Podcasts: /podcasts/slug/
              let docPath = strippedUrl.replace(/^\/?podcasts\//, "");
              if (docPath.endsWith("/index") && docPath.split("/").length === 2) {
                docPath = docPath.replace(/\/index$/, "");
              }
              baseUrl = `/podcasts/${docPath}`;
              ...
              let collection: "posts" | "pages" | "projects" | "podcasts" = "posts";
              if (file && (file as any).path) {
                ...
                else if (normalizedPath.includes("/podcasts/")) collection = "podcasts";
              }
  ...
  while ((markdownMatch = markdownLinkRegex.exec(content)) !== null) {
    ...
    if (isInternalLink(url)) {
      ...
      if (linkText) {
        ...
        // Note: Other content types (pages, projects, podcasts) are processed for URL conversion
  ...
  return function transformer(tree: any, file: any) {
    visit(tree, "image", (node: any) => {
      ...
      if (file.path) {
        ...
        // Check for podcasts
        else if (normalizedPath.includes("/podcasts/")) {
          collection = "podcasts";
          const docsIndex = pathParts.indexOf("podcasts");
          isFolderBased = normalizedPath.endsWith("/index.md");
