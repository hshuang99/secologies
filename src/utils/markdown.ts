...
// Matches CJK ideographs/kana/hangul, which don't use spaces between words.
// Each matched character is counted as one reading unit; everything else
// falls back to whitespace-delimited word counting.
const CJK_REGEX =
  /[\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\u3040-\u30ff\u30a0-\u30ff\uac00-\ud7af]/g;
...
function countCJKAndLatinWords(text: string): {
  cjkChars: number;
  latinWords: number;
} {
...
export function processMarkdown(content: string): {
  ...
  const { cjkChars, latinWords } = countCJKAndLatinWords(plainText);
  const wordCount = cjkChars + latinWords;

  // Create excerpt (first ~150 reading units or until first heading).
  // CJK text has no word boundaries, so slice by character count instead
  // of splitting on whitespace (which would return the whole paragraph
  // as a single "word").
  let excerpt: string;
  let hasMore: boolean;
  if (cjkChars > latinWords) {
    const EXCERPT_CHARS = 150;
    hasMore = plainText.length > EXCERPT_CHARS;
    excerpt = plainText.slice(0, EXCERPT_CHARS);
  } else {
    const words = plainText.split(/\s+/).filter((word) => word.length > 0);
    hasMore = words.length > 150;
...
export function calculateReadingTime(content: string): ReadingTime {
  ...
  // Average reading speed: ~225 English words/minute, ~300 CJK
  // characters/minute (CJK characters carry more info per glyph, so
  // this is slower than English wpm but faster than a naive per-word split).
  const LATIN_WORDS_PER_MINUTE = 225;
...
export function getReadingTime(
  ...
  // The remark reading-time plugin also splits on whitespace, so it
  // undercounts CJK content the same way the old manual calculation did.
  // For CJK content, always use our own CJK-aware calculation instead.
  const contentHasCJK = content !== undefined && hasCJK(content);
...
// Process content data for display (posts, projects, podcasts, etc.)
...
// Get next and previous podcast items within the same category, sorted by order
export function getAdjacentDocs<T extends { id: string; data: { category?: string | null; order: number; title: string } }>(
  podcasts: T[],
  currentSlug: string,
  currentCategory: string | null
) {
  // Filter podcasts by the same category
  const categoryDocs = podcasts.filter((doc) => {
    ...
    // If current doc has no category, match podcasts with no category
    if (!currentCategory) {
      return !docCategory;
    }
  ...
  // Only return navigation if there are other podcasts in the category
